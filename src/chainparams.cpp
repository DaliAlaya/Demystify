...
public:
	CMainParams() {
		...
		genesis = CreateGenesisBlock(1540650869, 227618, 0x1e0ffff0, 1,
				20 * COIN);
		...
		consensus.nPowTargetTimespan = 24 * 60 * 60; // Demystify: 1 day
		consensus.nPowTargetSpacing = 2 * 60; // Demystify: 2.5 minutes
		...
		consensus.vDeployments[Consensus::DEPLOYMENT_CSV].nStartTime =
				1540650869;
		consensus.vDeployments[Consensus::DEPLOYMENT_CSV].nTimeout = 1540650870;
		...
		consensus.hashGenesisBlock = genesis.GetHash();
			...
			== uint256S(
								"000006a25328f609e8500979f3d2d6ee7a07d86807b00fa6fbe4d368c3541998"));
		vSeeds.push_back(CDNSSeedData("dns1.dnsdemystify.xyz", "dns1.dnsdemystify.xyz"));
		vSeeds.push_back(CDNSSeedData("dns2.dnsdemystify.xyz", "dns2.dnsdemystify.xyz"));
		vSeeds.push_back(CDNSSeedData("dns3.dnsdemystify.xyz", "dns3.dnsdemystify.xyz"));
		...
		// privKey: DdH6467yrBNwg3fbynaUkT1JdSSjiqv8ZViwKRLfgqrjMSLRVY61
		strSporkAddress = "doJofSuGrhUkGLjUZpckMfqh9wvM48AK7j";
		...
		//  Demystify addresses start with 'd'
		// std::cout<< "test :" << base58Prefixes[PUBKEY_ADDRESS] ;
		base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 91);
		//  Demystify script addresses start with '2'
		base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 4);

		//  private keys start with 'D'
		base58Prefixes[SECRET_KEY] = std::vector<unsigned char>(1, 85);
		//  Demystify BIP32 pubkeys start with 'tpub'
		base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x04)(0x39)(
				0x88)(0xA1).convert_to_container<std::vector<unsigned char> >();
		//  Demystify BIP32 prvkeys start with 'tprv'
		base58Prefixes[EXT_SECRET_KEY] = boost::assign::list_of(0x04)(0x38)(
				0x77)(0xB2).convert_to_container<std::vector<unsigned char> >();

		//  Demystify BIP44 coin type is '1' (All coin's testnet default)
		nExtCoinType = 1;
	}
...
//		}
		...
		consensus.nPowTargetTimespan = 24 * 60 * 60; // Demystify: 1 day
		consensus.nPowTargetSpacing = 60 * 60; // Demystify: 2.5 minutes
		...
		vSeeds.push_back(
				CDNSSeedData("dns1.testnet.dnsdemystify.xyz",
						"dns1.testnet.demystify.xyz"));

		vFixedSeeds = std::vector < SeedSpec6
				> (pnSeed6_test, pnSeed6_test + ARRAYLEN(pnSeed6_test));

		// Testnet Demystify addresses start with 'J'
		base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 106);
		// Testnet Demystify script addresses start with 'J' or 'j'
...
public:
	CDevNetParams() {
		...
		vFixedSeeds.clear();
		vSeeds.clear();
		//vSeeds.push_back(CDNSSeedData("demystifyevo.org",  "devnet-seed.demystifyevo.org"));

		// Testnet Demystify addresses start with 'y'
		base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 106);
		// Testnet Demystify script addresses start with '8' or '9'
		base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 19);
	...
	CRegTestParams() {
		...
		consensus.powLimit =
				uint256S(
						"7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~uint256(0) >> 1
		consensus.nPowTargetTimespan = 24 * 60 * 60; // Demystify: 1 day
		consensus.nPowTargetSpacing = 2.5 * 60; // Demystify: 2.5 minutes
		consensus.fPowAllowMinDifficultyBlocks = true;
		...
		// Regtest Demystify addresses start with 'y'
		base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 140);
		// Regtest Demystify script addresses start with '8' or '9'
		base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 19);
		// Regtest private keys start with '9' or 'c' (Bitcoin defaults)
		base58Prefixes[SECRET_KEY] = std::vector<unsigned char>(1, 239);
		// Regtest Demystify BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
		base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x04)(0x35)(
				0x87)(0xCF).convert_to_container<std::vector<unsigned char> >();
		// Regtest Demystify BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
		base58Prefixes[EXT_SECRET_KEY] = boost::assign::list_of(0x04)(0x35)(
				0x83)(0x94).convert_to_container<std::vector<unsigned char> >();

		// Regtest Demystify BIP44 coin type is '1' (All coin's testnet default)
		nExtCoinType = 1;
	}
