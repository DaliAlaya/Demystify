...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in demystify.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("demystify.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Demystify features */
    { "demystify",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "demystify",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "demystify",               "gobject",                &gobject,                true,  {} },
    { "demystify",               "voteraw",                &voteraw,                true,  {} },
