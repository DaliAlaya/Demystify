...
const char * const BITCOIN_CONF_FILENAME = "demystify.conf";
const char * const BITCOIN_PID_FILENAME = "demystifyd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "demystify" is a composite category enabling all Demystify-related debug output
                if(ptrCategory->count(std::string("demystify"))) {
...
#else
    const char* pszModule = "demystify";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DemystifyCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DemystifyCore
    // Mac: ~/Library/Application Support/DemystifyCore
    // Unix: ~/.demystifycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DemystifyCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DemystifyCore";
#else
    // Unix
    return pathRet / ".demystifycore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty demystify.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override demystify.conf
