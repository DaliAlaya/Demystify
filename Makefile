...
pkgdatadir = $(datadir)/demystifycore
pkgincludedir = $(includedir)/demystifycore
pkglibdir = $(libdir)/demystifycore
pkglibexecdir = $(libexecdir)/demystifycore
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/demystify-config.h.in \
	$(srcdir)/libdemystifyconsensus.pc.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/demystify-config.h
CONFIG_CLEAN_FILES = libdemystifyconsensus.pc share/setup.nsi \
...
ACLOCAL = ${SHELL} /home/dali/today/Demystify/build-aux/missing aclocal-1.14
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /home/dali/today/Demystify/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /home/dali/today/Demystify/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /home/dali/today/Demystify/build-aux/missing automake-1.14
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = demystify-cli
BITCOIN_DAEMON_NAME = demystifyd
BITCOIN_GUI_NAME = demystify-qt
BITCOIN_TX_NAME = demystify-tx
...
COPYRIGHT_HOLDERS_FINAL = The Demystify Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = Demystify Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /home/dali/today/Demystify/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = demystifycore
PACKAGE_BUGREPORT = https://github.com/demystifypay/demystify/issues
PACKAGE_NAME = Demystify Core
PACKAGE_STRING = Demystify Core 0.12.3
PACKAGE_TARNAME = demystifycore
PACKAGE_URL = https://demystify.com/
...
ZMQ_CFLAGS =  
ZMQ_LIBS = -lzmq  
abs_builddir = /home/dali/today/Demystify
abs_srcdir = /home/dali/today/Demystify
abs_top_builddir = /home/dali/today/Demystify
abs_top_srcdir = /home/dali/today/Demystify
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/dali/today/Demystify/build-aux/install-sh
...
pkgconfig_DATA = libdemystifyconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/demystify-cli.bash-completion \
	       $(top_srcdir)/contrib/demystify-tx.bash-completion \
	       $(top_srcdir)/contrib/demystifyd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_demystify_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_demystify_coverage.info test_demystify.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Demystify-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/demystify-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/demystify-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/demystify-config.h
$(top_srcdir)/src/config/demystify-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/demystify-config.h src/config/stamp-h1
libdemystifyconsensus.pc: $(top_builddir)/config.status $(srcdir)/libdemystifyconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/Demystify-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Demystify-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Demystify-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_demystify.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_demystify -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_demystify_filtered.info: test_demystify.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_demystify_filtered.info
...
#test_demystify_coverage.info: baseline_filtered_combined.info test_demystify_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_demystify_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_demystify_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_demystify_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_demystify.coverage/.dirstamp:  test_demystify_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_demystify.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_demystify.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
